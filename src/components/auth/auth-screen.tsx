...
import { Lock, Mail, Loader2, ShieldCheck, ExternalLink, Settings, CheckCircle2, AlertCircle } from 'lucide-react';
...
export function AuthScreen() {
  ...
  const [apiError, setApiError] = useState<{ 
    message: string; 
    link: string; 
    type: 'enable' | 'blocked' | 'config' | 'mfa'
  } | null>(null);
  ...
  const handleAuthError = (error: any) => {
    ...
    if (errorCode.includes('identity-toolkit-api-has-not-been-used') || errorCode.includes('api-not-enabled')) {
      ...
      setApiError({
        type: 'blocked',
        message: 'A sua Chave de API está restringindo o acesso. Mude para "Não restringir" no painel do Google.',
      ...
      setApiError({
        type: 'config',
        message: 'O login do Google não está ativo no Firebase. Ative a verificação em 2 etapas primeiro se o Google pedir.',
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4">
      ...
      <Card className="w-full max-w-md shadow-2xl border-none bg-card/80 backdrop-blur-md">
        ...
        <CardContent className="space-y-6">
          {apiError && (
            <Alert variant="destructive" className="bg-destructive/5 text-destructive border-destructive/20 animate-in fade-in duration-300">
              <AlertCircle className="h-4 w-4" />
              <AlertTitle className="font-bold">Ajuste Necessário</AlertTitle>
              <AlertDescription className="space-y-3">
                ...
                <p className="text-[10px] text-muted-foreground text-center italic">
                  Após corrigir, aguarde 5 minutos e atualize esta página.
                </p>
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center text-xs uppercase text-muted-foreground">
              <span className="bg-card px-3 font-semibold">Alternativa</span>
            </div>
          </div>

          <Button 
            variant="outline" 
            className="w-full h-12 font-semibold border-primary/20 hover:bg-primary/5 transition-all" 
        ...
        <CardFooter className="flex flex-col pt-2 border-t mt-4">
      ...
      <div className="mt-12 flex items-center gap-3 p-3 bg-green-500/10 rounded-full px-6 border border-green-500/20">
        <CheckCircle2 className="h-4 w-4 text-green-500" />
        <span className="text-xs text-green-700 uppercase tracking-widest font-bold">Criptografia de Ponta a Ponta Ativa</span>
      </div>
    </div>
  );
}
