...
export function AuthScreen() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast({ title: "Bem-vindo!", description: "Acesso liberado com Google." });
    } catch (error: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 max-w-md mx-auto">
      <div className="mb-8 text-center space-y-2 animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-muted-foreground text-lg italic">Privacidade Total</p>
      </div>
      ...
      <Card className="w-full shadow-2xl border-none bg-card/80 backdrop-blur-md overflow-hidden ring-1 ring-black/5">
        <CardHeader className="space-y-1 text-center pb-8 bg-primary/5 border-b">
          ...
          <CardDescription>
            Acesse seus arquivos com segurança máxima.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 pt-8">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase text-muted-foreground">
              <span className="bg-card px-3 font-semibold tracking-widest">Ou use e-mail</span>
            </div>
          </div>
        ...
        <CardFooter className="flex flex-col pt-4 border-t bg-muted/20">
          <Button variant="link" className="text-sm text-primary" onClick={() => setIsLogin(!isLogin)}>
            {isLogin ? 'Não tem conta? Crie um agora' : 'Já tem conta? Faça login'}
          </Button>
        </CardFooter>
      </Card>
